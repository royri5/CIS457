...
void fill_window() {
    ...
    for(int i=0; i < WINDOW_SIZE; i++) {
        ...
        if(buffer_index == current_pkt_index) {
            ...
            window[window_end].seqnum = current_seq;
...
void A_input(struct pkt packet) {
    ...
    //      next_window_start = 0;
    // }
    // seq num max 50, wrapping, window = 4
    //if(packet.acknum < window[window_start].seqnum || packet.acknum > window[window_end].seqnum) {
    if(packet.acknum < min(window[window_start].seqnum, window[window_end].seqnum) || packet.acknum > max(window[window_start].seqnum, window[window_end].seqnum)) {
        // out of window
        printf("A: ACK out of window\n");
        return;
    }

    int diff = packet.acknum - window[window_start].seqnum;
    int next_window_start = window_start + diff + 1;
    if(next_window_start >= WINDOW_SIZE) {
